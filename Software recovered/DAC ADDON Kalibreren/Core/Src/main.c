...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define MAX4820_CS_PIN    GPIO_PIN_6   // De GPIO pin die verbonden is met CS
#define MAX4820_CS_PORT   GPIOB        // De GPIO poort die verbonden is met CS

#define DAC_SS_Pin GPIO_PIN_5
...
char String[] = "12345";
uint32_t convertedString;
uint16_t calData16bits;
uint32_t calData32bits;
uint8_t Select5V60V;
/* USER CODE END PV */
...
void MAX4820_SendData(uint8_t data)
{
    ...
    HAL_SPI_Transmit(&hspi1, txData, 1, HAL_MAX_DELAY);  // Verzenden van 8 bits data
...
HAL_StatusTypeDef DAC80501DataVoltage5V(uint16_t voltage_mV)  //data(getal) moet in mV zijn van 0 tot 5000mV
{
	...
	if(convertedString == 100002){
				 MAX4820_5V_pos();
			} else if(convertedString == 100004){
				MAX4820_5V_neg();
			}
...
HAL_StatusTypeDef CalibratedDAC80501DataVoltage60V(uint32_t voltage_mV)  //data(getal) moet in mV zijn van 0 tot 60000mV
{
	if(convertedString == 100001){
			 MAX4820_60V_pos();
		} else if(convertedString == 100003){
			MAX4820_60V_neg();
		}
	//Zorgen dat spanning tussen 0-60000mV zit
	if(voltage_mV > 60000){ //Houdt de spanning onder de 5000mV
		voltage_mV = 0;
	}
	if(voltage_mV < 0){ //Houdt de spanning boven de 0mV. Werkt niet goed vanwerge unsigned naar signed conversion bij functie aanroep
    ...
    calData32bits = (uint32_t)(float)(( 0.9205486)*convertedDataVoltage );
    if(calData32bits > 65535){ //Houdt de spanning onder de 5000mV
    	calData16bits = 65535;
          } else {
        	  calData16bits = calData32bits;
          }
    uint8_t txData[3];

//    0.9205486*x - 0.8901536

    // Stel het adres en de data in voor het trigger-register van de DAC80501
    txData[0] = ADDR_DAC_DATA_register;                     // Registeradres
    txData[1] = (calData16bits >> 8 ) & 0xFF;      // MSB van de 16-bits data
    txData[2] = (calData16bits & 0xFF);              // LSB van de 16-bits data
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size){
	...
	convertedString = (uint32_t)atoi((char* )RxBuf);

	//DAC80501DataVoltage60V(convertedString);
	if(convertedString == 100001 || convertedString == 100003) { Select5V60V = 1;}
	if(Select5V60V == 1){CalibratedDAC80501DataVoltage60V(convertedString);}
//	if(convertedString == 100001 || convertedString == 100003) { Select5V60V = 1;}
//	if(Select5V60V == 1){DAC80501DataVoltage60V(convertedString);}


	if(convertedString == 100002 || convertedString == 100004) { Select5V60V = 0;}
	if(Select5V60V == 0){DAC80501DataVoltage5V(convertedString);(convertedString);}


	//DAC80501DataVoltage5V(convertedString);
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOB, SS_DAC_Pin|SS_relais_Pin, GPIO_PIN_SET);
  ...
  /*Configure GPIO pins : SS_DAC_Pin SS_relais_Pin */
  GPIO_InitStruct.Pin = SS_DAC_Pin|SS_relais_Pin;
