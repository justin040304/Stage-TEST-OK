...
HAL_StatusTypeDef DAC80501DataVoltage5V(uint16_t voltage_mV)  //data(getal) moet in mV zijn van 0 tot 5000mV
{
    ...
    status = HAL_SPI_Transmit(&hspi3, txData, 3, DAC80501_SPI_TIMEOUT);
...
int main(void)
{
  ...
  uint8_t txData[3];
    uint16_t data = 65000;
    txData[0] = 0b00001000;                     // Registeradres
    txData[1] = (data >> 8 ) & 0xFF;      // MSB van de 16-bits data
    txData[2] = (data & 0xFF);              // LSB van de 16-bits data
...
static void MX_SPI3_Init(void)
{
  ...
  GPIO_InitStruct.Pin = GPIO_PIN_10; // GPIOC Pin 10 is the SPI3_CLK
  ...
  GPIO_InitStruct.Alternate = GPIO_AF6_SPI3; // Correcte alternate functie voor SPI3
